...
in
{
  wayland.windowManager.hyprland.settings.on = [
    ...
    {
      _args = [ "window.close" compactEvent ];
    }
    {
      _args = [ "window.move_to_workspace" compactEvent ];
    }
    {
      _args = [ "workspace.active" compactEvent ];
    }
  ];
}
